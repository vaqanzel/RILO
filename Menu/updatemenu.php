...
if(isset($_GET['idMenu'])) {
    $idMenu = $_GET['idMenu'];
    $qMenu = mysqli_query($con,"SELECT * FROM menu WHERE idMenu ='$idMenu'");
    $recMenu = mysqli_fetch_array($qMenu);
    $idKategoriMenu = $recMenu['idKategoriMenu'];
}
    ...
    <div class="container-fluid">
        ...
        <form action="../data.php" method="POST" >
                <div class="input-group input-group-outline mb-3">
                  ...
                  <input type="text" name="namaMenu" id="" class="form-control" placeholder="" value="<?= $recMenu['namaMenu'] ?>">
                </div>
                <div class="input-group input-group-outline mb-3">
                  <label class="form-label"></label>
                  <span class="input-group-text" id="basic-addon1">Rp.</span>
                  <input type="text" name="Harga" id="" class="form-control" placeholder="" value="<?= $recMenu['Harga'] ?>">
                </div>
                <div class="input-group input-group-outline mb-3">
                    <select name="idKategoriMenu" class="custom-select">
                        <?php 
                            $dkat = mysqli_query($con,"SELECT * FROM kategorimenu WHERE idKategoriMenu = '$idKategoriMenu'");
                            while ($bkat= mysqli_fetch_array($dkat)){
                        ?>
                        <option selected><?= $bkat['namaKategori'] ?></option>
                        <?php }?>
                        <?php
                ...
                <input type="hidden" name="idMenu" value="<?= $recMenu['idMenu'] ?>" />

                <input type="submit" style="background-color : #4F06B8;" class="btn float-end text-white" name="btnUpdateMenu" value="Update Menu" >
                </form>
            </div>
        </div>
    </div>
